...
extension PullToRefreshView where Header == MgcRefreshHeader, Content: View, Footer == MgcRefreshFooter {
    init(_ noMoreData: Binding<Bool>, @ViewBuilder content: @escaping () -> Content) {
        self.init(header: MgcRefreshHeader(), footer: MgcRefreshFooter(noMoreData: noMoreData), content: content)
    }
}
